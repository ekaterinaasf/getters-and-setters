  ...
  return {
    path:
      dirPath
        ...
        .split("/")
        .join("") + "/",
    files: arrayOfFiles.length > 0 ? arrayOfFiles : null,
    dirs: arrayOfDirs.length > 0 ? arrayOfDirs : null
  };
};
...
const evaluateFile = path => {
  ...
  const loopProtectedCode = rawCode.replace(
    /for *\(.*\{|while *\(.*\{|do *\{/g,
    loopHead => {
      number_of_loops++;
  ...
  try {
    ...
    if (typeof thrown === "string") {
      ...
      report.push({
        error: thrown.stack.split(__dirname).join(" [...] ")
      });
    }
  }
...
const evaluateDirectory = (toReport, path) => {
  ...
  const dirs = toReport.dirs
    ? toReport.dirs.map(dir => evaluateDirectory(dir, path + dir.path))
    : toReport.dirs;

  const statusOf = entries =>
    entries
      ? entries.reduce(
          (stat, entry) => (entry.status > stat ? entry.status : stat),
          -1
        )
      : -1;
  ...
  const status = dirsStatus >= filesStatus ? dirsStatus : filesStatus;
...
const generateFileSectionMd = fileReport => {
  ...
  const renderedReport = fileReport.report
    .map(entry => {
      if (entry.error) {
        return entry.error.includes("SyntaxError") ? entry.error : entry.error;
      }
      if (entry.warning) {
        return "warning: " + entry.warning;
      }
      if (entry.assertion) {
        const assertion = entry.assertion[0] ? "+ PASS: " : "- FAIL: ";
        const message = entry.assertion.slice(1).toString();
        return assertion + message;
      }
    })
    .reduce((all, next) => all + next + "\n", "");

  const report = renderedReport ? "```txt\n" + renderedReport + "```\n\n" : "";
...
const generateREVIEWs = report => {
  if (report.dirs) {
    report.dirs.forEach(report => generateREVIEWs(report));
  }

  const dirName = path => {
    const pathArr = path.slice(0, path.length - 1).split("/");
  ...
  const fileList = report.files
    ? report.files
        .map(file => {
          ...
          return `* [${relPath}](#${localHref}---${interpret(
            "status",
            file.status
          )
            .split(" ")
            .join("-")}) - ${interpret("status", file.status)}`;
        })
        .reduce((list, li) => list + li + "\n", "")
    : "";

  const filesIndex = fileList ? `### files\n\n` + fileList + "\n" : "";

  const dirList = report.dirs
    ? report.dirs
        .map(dir => {
          const relPath = dirName(dir.path);
          return `* [${relPath}](./${relPath}REVIEW.md) - ${interpret(
            "status",
            dir.status
          )}`;
        })
  ...
  const subDirIndex = dirList ? `### sub-directories\n\n` + dirList + "\n" : "";

  const index = "* [../REVIEW.md](../REVIEW.md)\n\n" + filesIndex + subDirIndex;
  ...
  const newREVIEW = top + index + fileSections;
...
const cleanedTopLevelREVIEWSource = topLevelREVIEWSource
  ...
  .join("");
fs.writeFile("./REVIEW.md", cleanedTopLevelREVIEWSource, err => {
  if (err) {
    console.log(err);
  }
});
